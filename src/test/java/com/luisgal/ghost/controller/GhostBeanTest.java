...
public class GhostBeanTest {
  private class MovementMatcher extends ArgumentMatcher<GameMovementDTO> {
    ...
    public MovementMatcher(final String prefix, final Character input) {
      if ((prefix == null) || (input == null)) {
        throw new IllegalArgumentException(
            "Null arguments not allowed in constructor.");
      } else {
        this.prefix = prefix;
        this.input = input;
      }
    ...
    public boolean matches(Object argument) {
      if (argument instanceof GameMovementDTO) {
        ...
        return (prefix.equals(movement.getOldPrefix()) && input.equals(movement
            .getNewInput()));
      } else {
        return false;
      }
  ...
  private void testPlayerInput(final GameRating rating) {
    ...
    // verify
    verify(facade).getNextState(
        argThat(new MovementMatcher(oldPrefix, newInput)));
