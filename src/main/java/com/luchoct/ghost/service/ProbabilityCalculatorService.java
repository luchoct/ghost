...
public class ProbabilityCalculatorService implements ProbabilityCalculator {
	...
	public final float getNextPlayerProbability(final MetricsDTO metrics) {
		...
		final float weightInputs = getWeightInputs(metrics.getWinnerSuffixes().size()
				+ metrics.getLoserSuffixes().size()
				- metrics.getWinnerInputs().size()
				- metrics.getLoserInputs().size());
		//We use the size of the loser inputs, because a lost for previous player is a win for next player
		final float probabilityInputs =
				getProportionalRatio(metrics.getLoserInputs().size(), metrics.getWinnerInputs().size()) * weightInputs;
		...
		final float probabilitySuffixes =
				getProportionalRatio(metrics.getLoserSuffixes().size(),
						metrics.getWinnerSuffixes().size()) * weightSuffixes;

		return probabilityInputs + probabilitySuffixes;
	}

	/**
	 * Returns the weight of inputs. It´s between 75 and 100, and when it increases when the number of suffixes
	 * decreases.
